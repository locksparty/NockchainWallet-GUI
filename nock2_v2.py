...
"""
Nockchain Wallet Qt Interface - Version 2
Interface graphique pour le wallet Nockchain avec support des transactions
Version 2: simplified _build_base_command (keeps signature for compatibility)
"""
    ...
    def send_funds(self, recipient: str, amount_nicks: int, note_names: List[str], fee: int = 10) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "cmd": " ".join(e.cmd) if hasattr(e, 'cmd') else ''
            }
        except Exception as e:
            return {
                "success": False,
...
class NockchainWalletGUI(QMainWindow):
    ...
    def _build_base_command(self, include_wallet: bool = True) -> list:
        ...
        Le binaire gère le wallet chargé en mémoire et le serveur public par défaut,
        donc on n'ajoute des flags que pour le mode 'private'.
        Le paramètre include_wallet est conservé pour compatibilité avec les appels existants.
        """
        cmd = [config['wallet_binary']]

        # Mode privé : on ajoute les flags nécessaires (port si différent du défaut)
        if config.get('client_type') == 'private':
            cmd.extend(['--client', 'private'])
            if config.get('private_port') and config.get('private_port') != '50051':
                cmd.extend(['--private-grpc-server-port', config['private_port']])

        # Note: pour le client public, et pour le wallet, le binaire se débrouille seul,
        # donc nous n'ajoutons pas --client public ni --wallet.
        return cmd
    ...
    def _check_binary(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.log_area.append_log("⚠ Binaire non fonctionnel", "warning")
        except FileNotFoundError:
            self.log_area.append_log(f"✗ Binaire non trouvé: {config['wallet_binary']}", "error")
            QMessageBox.warning(self, "Binaire manquant",
    ...
    def _select_wallet(self):
        ...
        wallet_file, _ = QFileDialog.getOpenFileName(self, "Sélectionner un wallet",
    ...
    def _import_wallet(self):
        ...
        source_file, _ = QFileDialog.getOpenFileName(self, "Sélectionner le fichier de clés",
                                                     config['last_used_directory'],
    ...
    def _export_wallet(self):
        ...
        output_file, _ = QFileDialog.getSaveFileName(self, "Exporter les clés du wallet",
        ...
        try:
            cmd = [config['wallet_binary'], 'export-keys', '--wallet', config['wallet_path'], '--output', output_file]
    ...
    def _browse_binary(self):
        ...
        binary_file, _ = QFileDialog.getOpenFileName(self, "Sélectionner le binaire nockchain-wallet",
